        ...
        try:
            ...
            for value in parts:
                try:
                    ...
                    print(f"Error on parameter '{value.strip()}': could\
 not convert string to float")
                    break


def calcul_dist(p1: tuple[float, float, float],
                p2: tuple[float, float, float] = (0.0, 0.0, 0.0)) -> float:
...
def main() -> None:
    ...
    print(f"It includes: X={coordinates_1[0]}, Y={coordinates_1[1]},\
 Z={coordinates_1[2]}")
